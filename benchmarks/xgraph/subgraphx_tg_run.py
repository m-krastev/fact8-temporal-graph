...
def pipeline(config: DictConfig):
    ...
    if (
        config.explainers.explainer_name == "subgraphx_tg"
        and config.explainers.parallel_degree == 1
    ):
        ...
        explain_results = start_multi_process(
            explainer, target_event_idxs, config.explainers.parallel_degree
        )
    else:
        explain_results = explainer(event_idxs=target_event_idxs)
    end_time = time.time()
