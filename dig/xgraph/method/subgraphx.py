...
class MCTS(object):
    ...
    def mcts_rollout(self, tree_node):
        cur_graph_coalition = tree_node.coalition # all nodes of this graph
        ...
        if len(tree_node.children) == 0:
            ...
            for each_node in expand_nodes:
                ...
                subgraphs = [self.graph.subgraph(c) # all connected components
                             for c in nx.connected_components(self.graph.subgraph(subgraph_coalition))]

                if self.new_node_idx: # select one connected component which contains the new_node_idx
                    for sub in subgraphs:
                        if self.new_node_idx in list(sub.nodes()):
                            main_sub = sub
                else: # or only select the largest connected component
...
class SubgraphX(object):
    ...
    Args:
        ...
        num_hops(:obj:`int`, :obj:`None`): The number of hops to extract neighborhood of target node
          (default: :obj:`None`)
        explain_graph(:obj:`bool`): Whether to explain graph classification model (default: :obj:`True`), or node-level explaination
    ...
    def explain(self, x: Tensor, edge_index: Tensor, label: int,
        ...
        if self.explain_graph: # graph-level explanation or node-level explanation
