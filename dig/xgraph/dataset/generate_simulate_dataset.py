...
def generate_dataset(hawkes_list):
    positive_hawkes = []
    negative_hawkes = []
    for hawkes in hawkes_list:
        if positive_check(hawkes):
            positive_hawkes.append(hawkes)
        else:
            negative_hawkes.append(hawkes)
    
    random.shuffle(negative_hawkes)
    negative_hawkes = negative_hawkes[: len(positive_hawkes)+1]
    data_hawkes = positive_hawkes + negative_hawkes
    labels = [1]*len(positive_hawkes) + [0]*len(negative_hawkes)
    data_hawkes, labels = shuffle(data_hawkes, labels)

    dataset_events = []
    for hawkes in data_hawkes:
        dataset_events.append( generate_events(hawkes) )
    
    print('original length avg:', np.mean(list(map(lambda x: len(x), dataset_events))))
    
    pos_end_idx = []
    neg_end_idx = []
    for l, events in zip(labels, dataset_events):
        end_idx = compute_end_idx(events)
        if l == 0:
            pos_end_idx.append(end_idx)
        else:
            neg_end_idx.append(end_idx)
    
    print(f'positive end idx avg: {np.mean(pos_end_idx):.2f}')
    print(f'negative end idx avg: {np.mean(neg_end_idx):.2f}')

    for i, events in enumerate(dataset_events):
        end_idx = compute_end_idx(events)
        dataset_events[i] = dataset_events[i][:end_idx+1]
    
    return dataset_events, labels
...
def make_dataset_v1(hawkes, type_edge_mapping, num_nodes, dataset_name):
    # target_type = 2
    target_type = 3
...
if __name__ == '__main__':
    ...
    make_dataset_v1(hawkes, type_edge_mapping, n_nodes, dataset_name=dataset_name)
